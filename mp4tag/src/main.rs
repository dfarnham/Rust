    ...
    for file in m4a_files {
        ...
        if let Some(trkn) = args.get_one::<String>("trkn") {
            match trkn.is_empty() {
                ...
                false => {
                    let trim = true;
                    let components = split_on::<u16>(trkn, '/', trim)?;
                    assert_eq!(components.len(), 2, "expected track_number/total_tracks");
                    let (track_number, total_tracks) = (components[0], components[1]);
                    assert!(total_tracks >= track_number, "expected total_tracks >= track_number");
